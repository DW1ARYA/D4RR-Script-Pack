-- Error Check
if DiscordWebhookSystemInfos == nil and DiscordWebhookPlayerlogs == nil and DiscordWebhookKillinglogs == nil and DiscordWebhookChat == nil then
...
if DiscordWebhookPlayerlogs == 'WEBHOOK_LINK_HERE' then
	print('\n\nERROR\n' .. GetCurrentResourceName() .. ': Please add your "Player logs" webhook\n\n')
else
	PerformHttpRequest(DiscordWebhookPlayerlogs, function(Error, Content, Head)
		if Content == '{"code": 50027, "message": "Invalid Webhook Token"}' then
			print('\n\nERROR\n' .. GetCurrentResourceName() .. ': "Player logs" webhook non-existing!\n\n')
		end
	end)
end
...
-- System Infos
PerformHttpRequest(DiscordWebhookSystemInfos, function(Error, Content, Head) end, 'POST', json.encode({username = SystemName, content = '**Server Online!**'}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('playerConnecting', function()
	...
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName, '`' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```css\n'.. GetPlayerName(source) ..'["' .. GetPlayerIdentifier(source) .. '"] connecting\n```', SystemAvatar, false)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName, '`DATE :' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```css\n'.. GetPlayerName(source) ..' connecting\n["steam:' .. GetPlayerIdentifier(source) .. '"]\n["'.. identifierDiscord ..'"]```', SystemAvatar, false)
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName,'' .. GetPlayerName(source) ..'[' .. GetPlayerIdentifier(source) .. '] connecting', SystemAvatar, false)
end)
...
AddEventHandler('playerDropped', function(Reason)
	...
	if date.min < 10 then date.min = '0' .. tostring(date.min) end
	if date.sec < 10 then date.sec = '0' .. tostring(date.sec) end
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName, '`' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```fix\n'.. GetPlayerName(source) ..'["' .. GetPlayerIdentifier(source) .. '"] left (' .. Reason .. ')\n```', SystemAvatar, false)
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName, '`DATE :' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```fix\n'.. GetPlayerName(source) ..' left the server\n["' .. GetPlayerIdentifier(source) .. '"]\n["'.. identifierDiscord ..'"]\n["Reason:' .. Reason .. '"]```', SystemAvatar, false)
	--TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookPlayerlogs, SystemName,'' .. GetPlayerName(source) ..'[' .. GetPlayerIdentifier(source) .. '] left (' .. Reason .. ')', SystemAvatar, false)
end)
...
AddEventHandler('DiscordBot:playerMati', function(Message, Weapon)
	...
	TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookKillinglogs, SystemName, '`DATE :' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`\n```\n' .. Message .. '\n```', SystemAvatar, false)
end)
