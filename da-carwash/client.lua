...
function da_carwash_DrawSubtitleTimed(m_text, showtime)
	SetTextEntry_2('STRING')
	AddTextComponentString(m_text)
	DrawSubtitleTimed(showtime, 1)
end

function da_carwash_DrawNotification(m_text)
		...
		if IsPedSittingInAnyVehicle(PlayerPedId()) then 
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					if mycie == false then
					HelpPromt('Press ~INPUT_CONTEXT~ to ~b~clean ~s~vehicle for ~g~($25000)')
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('da-carwash:checkmoney')
					end
				else
...
RegisterNetEvent('da-carwash:success')
AddEventHandler('da-carwash:success', function (price)
    ...
    Citizen.CreateThread(function()
		while timer2 do
			...
			elseif (timer == 0) then
				...
				TriggerEvent("mythic_notify:client:SendAlert", {type = 'inform', text = "Kendaraan bersih! biaya $25000"})
				StopParticleFxLooped(particles, 0)
				StopParticleFxLooped(particles2, 0)
				timer2 = false
			end
...
RegisterNetEvent('da-carwash:notenoughmoney')
AddEventHandler('da-carwash:notenoughmoney', function (moneyleft)
	...
	TriggerEvent("mythic_notify:client:SendAlert", {type = 'error', text = "Uang tidak cukup! membutuhkan:" .. moneyleft .."$ lagi."})
end)
